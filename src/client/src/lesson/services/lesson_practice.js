(function () {
  ...
  function LessonPracticeService(UserService) {
    var self = this;
    var service = {};
    ...
    service.query = function (lessons, error_container) {
      $http.get('/api/lesson/practice').success(function (data, status, headers, config) {
        console.log(data);
        if (data.status == 0) {
          lessons = data.data;
        } else {
  ...
  angular.module('sllp.lesson')
    .service('LessonPracticeService', ['UserService', LessonPracticeService]);
}());