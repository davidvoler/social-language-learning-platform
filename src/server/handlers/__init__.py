from server.handlers.profile_handler import ProfileHandler
from server.handlers.lesson_edit_handler import LessonEditHandler
from server.handlers.lesson_learn_handler import LessonLearnHandler
from server.handlers.lesson_practice_handler import LessonPracticeHandler
from server.handlers.index_handler import IndexHandler
from server.handlers.login_handler import LoginHandler
from server.handlers.google_oauth2_login_handler import GoogleOAuth2LoginHandler
from server.handlers.language_handler import LanguageHandler
from server.handlers.base_handler import BaseHandler
from server.handlers.gauth_login_handler import GAuthLoginHandler
from server.handlers.tag_handler import TagHandler
from server.handlers.app_handler import AppHandler


